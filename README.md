...
- 按 F 默认拾取整个选中 Group；客户端只发送代表 ItemEntity ID
- 服务端根据真实代表 Stack 重新扫描并验证 Group，不信任客户端缓存、数量或成员列表
- 每个成员分别验证距离、owner 和 pickup delay，并复用安全库存事务逐个拾取
- 背包容量不足时只扣除实际插入数量，剩余物品继续保留在地面
...
**当前仍处于早期开发阶段。** 当前版本尚不包含指定数量拾取、Shift + 滚轮数量调整、过滤器、大型战利品界面或 Forge/NeoForge 支持。
