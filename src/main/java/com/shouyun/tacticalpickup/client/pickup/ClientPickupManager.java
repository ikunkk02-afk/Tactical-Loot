...
public final class ClientPickupManager {
	...
	public boolean requestPickup(int representativeEntityId, int requestedAmount) {
		if (requestedAmount < PickupRequestPayload.ALL_ITEMS || Minecraft.getInstance().getConnection() == null) {
			return false;
		}

		PacketDistributor.sendToServer(new PickupRequestPayload(representativeEntityId, requestedAmount));
		requestScan();
		return true;
	}
	...
	public boolean requestPickupToSlot(int representativeEntityId, int requestedAmount, int targetSlot) {
		...
		PacketDistributor.sendToServer(new PickupToSlotRequestPayload(
	...
	public boolean requestDropInventorySlot(int sourceSlot) {
		...
		PacketDistributor.sendToServer(new DropInventorySlotPayload(sourceSlot, DropInventorySlotPayload.ALL_ITEMS));
		requestScan();
		return true;
	}
